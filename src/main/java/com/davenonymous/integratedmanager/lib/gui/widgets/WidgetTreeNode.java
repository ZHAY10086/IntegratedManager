...
import com.davenonymous.integratedmanager.lib.gui.ISelectable;
		...
		if(canExpand) {
			...
			if(count() <= 256) {
				...
				expander.setTooltipElements(
					WrappedStringTooltipComponent.yellow(I18n.get("integratedmanager.gui.files.overflow.label")),
					WrappedStringTooltipComponent.gray(I18n.get("integratedmanager.gui.files.overflow.tooltip", count()))
				);
			}
			box.add(expander);
